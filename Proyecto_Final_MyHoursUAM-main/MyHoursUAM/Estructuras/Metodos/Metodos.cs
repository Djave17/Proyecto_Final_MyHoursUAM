...
namespace MyHours_UAMApp.Estructuras.Metodos
{
    internal static class Metodos
    {
        ...
        /// </summary>
        ///<param name="partidosAsistidos "></param>
        /// <param name="eventosAsistidos"></param>

        public static DataTable ConvertirEventosAsistidosADataTable(List<Evento> eventosAsistidos)
        {
            DataTable tablaEventos = new DataTable();

            // Definir columnas basadas en las subcolumnas del ListView
            ...
            tablaEventos.Columns.Add("CantidadConvalidar", typeof(string));
            ...
            foreach (var evento in eventosAsistidos)
            {
                // Agregar datos en las mismas columnas usadas en el ListView
                tablaEventos.Rows.Add(
                    evento.idEvento,
                    evento.nombreEvento,
                    evento.tipoBeneficio,
                    evento.cantidadConvalidar,
                    evento.horaEvento,
                    evento.fechaEvento,
                    evento.lugarEvento,
        ...
        public static DataTable ConvertirPartidosAsistidosADataTable(List<Partido> partidosAsistidos)
        {
            ...
            tablaPartidos.Columns.Add("CantidadConvalidar", typeof(string));
            ...
            foreach (var partido in partidosAsistidos)
            {
                // Manejar nulos de la misma forma que en el ListView
                tablaPartidos.Rows.Add(
                    partido.idEvento ?? "N/A",
                    partido.nombrePartido ?? "N/A",
                    partido.cantidadConvalidar.ToString() ?? "0",
                    partido.horaEvento ?? "N/A",
                    partido.fechaEvento ?? "N/A",
                    partido.lugarPartido ?? "N/A",
        ...
        public static void ConfigurarReportViewer(ReportViewer reportViewer, List<Evento> eventosAsistidos, List<Partido> partidosAsistidos)
        {
            // Convertir listas a DataTables
            DataTable tablaEventos = ConvertirEventosAsistidosADataTable(eventosAsistidos);
            DataTable tablaPartidos = ConvertirPartidosAsistidosADataTable(partidosAsistidos);

            // Configurar el ReportViewer
            reportViewer.Reset();
            reportViewer.LocalReport.ReportPath = "RptInformeUsuario.rdlc"; // Ruta del archivo RDLC
            reportViewer.LocalReport.DataSources.Clear();

            // Agregar DataSources al ReportViewer
            reportViewer.LocalReport.DataSources.Add(new ReportDataSource("DsEvento", tablaEventos));
            reportViewer.LocalReport.DataSources.Add(new ReportDataSource("DsPartido", tablaPartidos));
